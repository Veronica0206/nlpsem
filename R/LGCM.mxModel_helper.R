...
#' Evaluated
#'
#' @description This function builds up an object of mxModel for a latent growth curve model with user-specified
...
getLGCM.mxModel <- function(dat, t_var, y_var, curveFun, intrinsic, records, growth_TIC, starts){
  ...
  if (!is.null(growth_TIC)){
    ...
    TIC_MEAN <- mxPath(from = "one", to = growth_TIC, arrows = 1, free = TRUE, values = starts$TIC_starts$means,
                       labels = paste0("mux", 1:nTICs))
    # Var-cov of TIC(s)
    TIC_VAR <- mxPath(from = growth_TIC, to = growth_TIC, connect = "unique.pairs", arrows = 2, free = TRUE,
                      values = starts$TIC_starts$covMatrix, labels = paste0("phi", 1:(nTICs * (nTICs + 1)/2)))
  }
    ...
    if (!is.null(growth_TIC)){
      ...
      model_mx <- mxModel(name = "LGCM_TICs, Linear Functional Form", type = "RAM",
                          ...
                          mxPath(from = latents[1:2], to = latents[1:2], arrows = 2, connect = "unique.pairs", free = TRUE,
                                 values = starts$Y_starts$covMatrix, labels = c("Y_psi00", "Y_psi01", "Y_psi11")),
                          ...
                          mxPath(from = paste0(y_var, records), to = paste0(y_var, records), arrows = 2, free = TRUE,
                                 values = starts$Y_starts$residuals, labels = "Y_residuals"),
                          mxAlgebra(rbind(Y_mueta0, Y_mueta1), name = "Y_alpha0"),
                          mxAlgebra(rbind(cbind(Y_psi00, Y_psi01),
                                          cbind(Y_psi01, Y_psi11)), name = "Y_psi_r"),
                          mxMatrix("Full", 2, length(growth_TIC), free = TRUE, values = starts$beta,
                                   labels = c(paste0("beta0", seq_along(growth_TIC)),
                                              paste0("beta1", seq_along(growth_TIC))), byrow = TRUE, name = "beta"),
                          mxMatrix("Full", length(growth_TIC), 1, free = TRUE, values = starts$TIC_starts$means,
    ...
    if (!is.null(growth_TIC)){
      ...
      model_mx <- mxModel(name = "LGCM_TICs, Quadratic Functional Form", type = "RAM",
                          ...
                          mxPath(from = latents[1:3], to = latents[1:3], arrows = 2, connect = "unique.pairs", free = TRUE,
                                 values = starts$Y_starts$covMatrix, labels = c("Y_psi00", "Y_psi01", "Y_psi02", "Y_psi11", "Y_psi12", "Y_psi22")),
                          mxPath(from = "eta0", to = paste0(y_var, records), arrows = 1, free = FALSE, values = 1),
                          mxPath(from = "eta1", to = paste0(y_var, records), arrows = 1, free = FALSE, values = 0,
                                 labels = paste0("L1", records, "[1,1]")),
                          ...
                          mxMatrix("Full", 3, length(growth_TIC), free = TRUE, values = starts$beta,
                                   labels = c(paste0("beta0", seq_along(growth_TIC)),
                                              paste0("beta1", seq_along(growth_TIC)),
                                              paste0("beta2", seq_along(growth_TIC))), byrow = TRUE, name = "beta"),
                          mxMatrix("Full", length(growth_TIC), 1, free = TRUE, values = starts$TIC_starts$means,
                                   labels = c(paste0("mux", seq_along(growth_TIC))), byrow = FALSE, name = "mux"),
                          mxAlgebra(Y_alpha0 + beta %*% mux, name = "Y_mean0"),
                          GF_loadings, TIC_MEAN, TIC_VAR, BETA)
  ...
  # Define mxModel for a LGCM with negative exponential functional form and TICs (if any)
    ...
    if (intrinsic){
      ...
      if (!is.null(growth_TIC)){
        ...
        BETA[[nGF]] <- mxPath(from = growth_TIC, to = latents[nGF], arrows = 1, free = TRUE, values = starts$beta[nGF, ],
        ...
        model_mx <- mxModel(name = "LGCM_TICs, Negative Exponential Functional Form, Random Ratio", type = "RAM",
                            ...
                            mxPath(from = "one", to = latents[1:2], arrows = 1, free = TRUE, values = starts$Y_starts$means[1:2],
                                   labels = c("Y_mueta0", "Y_mueta1")),
                            mxMatrix("Full", 1, 1, free = TRUE, values = starts$Y_starts$means[3], labels = "Y_slp_ratio", name = "Y_mug"),
                            mxPath(from = latents[1:3], to = latents[1:3], arrows = 2, connect = "unique.pairs", free = TRUE,
                                   values = starts$Y_starts$covMatrix, labels = c("Y_psi00", "Y_psi01", "Y_psi0g", "Y_psi11", "Y_psi1g", "Y_psigg")),
                            mxPath(from = "eta0", to = paste0(y_var, records), arrows = 1, free = FALSE, values = 1),
                            mxPath(from = "eta1", to = paste0(y_var, records), arrows = 1, free = FALSE, values = 0,
                                   labels = paste0("L1", records, "[1,1]")),
                            ...
                            mxMatrix("Full", 3, length(growth_TIC), free = TRUE, values = starts$beta,
                                     labels = c(paste0("beta0", seq_along(growth_TIC)),
                                                paste0("beta1", seq_along(growth_TIC)),
                                                paste0("betag", seq_along(growth_TIC))), byrow = TRUE, name = "beta"),
                            mxMatrix("Full", length(growth_TIC), 1, free = TRUE, values = starts$TIC_starts$means,
                                     labels = c(paste0("mux", seq_along(growth_TIC))), byrow = FALSE, name = "mux"),
                            mxAlgebra(Y_alpha0 + beta %*% mux, name = "Y_mean0"),
                            GF_loadings, TIC_MEAN, TIC_VAR, BETA)
    ...
    # fixed ratio of growth rate
      ...
      if (!is.null(growth_TIC)){
        ...
        model_mx <- mxModel(name = "LGCM_TICs, Negative Exponential Functional Form, Fixed Ratio", type = "RAM",
                            ...
                            mxAlgebra(rbind(cbind(Y_psi00, Y_psi01),
                                            cbind(Y_psi01, Y_psi11)), name = "Y_psi_r"),
                            mxMatrix("Full", 2, length(growth_TIC), free = TRUE, values = starts$beta[1:2, ],
                                     labels = c(paste0("beta0", seq_along(growth_TIC)),
                                                paste0("beta1", seq_along(growth_TIC))), byrow = TRUE, name = "beta"),
                            mxMatrix("Full", length(growth_TIC), 1, free = TRUE, values = starts$TIC_starts$means,
                                     labels = c(paste0("mux", seq_along(growth_TIC))), byrow = FALSE, name = "mux"),
      ...
      if (!is.null(growth_TIC)){
        ...
        model_mx <- mxModel(name = "LGCM_TICs, Jenss Bayley Functional Form, Random Ratio", type = "RAM",
                            ...
                            mxPath(from = "one", to = latents[1:3], arrows = 1, free = TRUE, values = starts$Y_starts$means[1:3],
                                   labels = c("Y_mueta0", "Y_mueta1", "Y_mueta2")),
                            mxMatrix("Full", 1, 1, free = TRUE, values = starts$Y_starts$means[4], labels = "Y_acc_ratio", name = "Y_mug"),
                            mxPath(from = latents[1:4], to = latents[1:4], arrows = 2, connect = "unique.pairs", free = TRUE,
                                   values = starts$Y_starts$covMatrix, labels = c("Y_psi00", "Y_psi01", "Y_psi02", "Y_psi0g", "Y_psi11",
                            ...
                            mxMatrix("Full", 4, length(growth_TIC), free = TRUE, values = starts$beta,
                                     labels = c(paste0("beta0", seq_along(growth_TIC)),
                                                paste0("beta1", seq_along(growth_TIC)),
                                                paste0("beta2", seq_along(growth_TIC)),
                                                paste0("betag", seq_along(growth_TIC))), byrow = TRUE, name = "beta"),
                            mxMatrix("Full", length(growth_TIC), 1, free = TRUE, values = starts$TIC_starts$means,
                                     labels = c(paste0("mux", seq_along(growth_TIC))), byrow = FALSE, name = "mux"),
                            mxAlgebra(Y_alpha0 + beta %*% mux, name = "Y_mean0"),
      ...
      if (!is.null(growth_TIC)){
        ...
        model_mx <- mxModel(name = "LGCM_TICs, Jenss Bayley Functional Form, Fixed Ratio", type = "RAM",
                            ...
                            mxAlgebra(rbind(cbind(Y_psi00, Y_psi01, Y_psi02),
                                            cbind(Y_psi01, Y_psi11, Y_psi12),
                                            cbind(Y_psi02, Y_psi12, Y_psi22)), name = "Y_psi_r"),
                            mxMatrix("Full", 3, length(growth_TIC), free = TRUE, values = starts$beta[1:3, ],
                                     labels = c(paste0("beta0", seq_along(growth_TIC)),
                                                paste0("beta1", seq_along(growth_TIC)),
                                                paste0("beta2", seq_along(growth_TIC))), byrow = TRUE, name = "beta"),
                            mxMatrix("Full", length(growth_TIC), 1, free = TRUE, values = starts$TIC_starts$means,
    ...
    if (intrinsic){
      ...
      if (!is.null(growth_TIC)){
        ...
        model_mx <- mxModel(name = "LGCM_TICs, Bilinear Spline Functional Form, Random knot", type = "RAM",
                            ...
                            mxMatrix("Full", 1, 1, free = TRUE, values = starts$Y_starts$means[4], labels = "knot", name = "Y_mug"),
                            mxPath(from = latents[1:4], to = latents[1:4], arrows = 2, connect = "unique.pairs", free = TRUE,
                                   values = starts$Y_starts$covMatrix, labels = c("Y_psi00s", "Y_psi01s", "Y_psi02s", "Y_psi0gs", "Y_psi11s",
                            ...
                            mxAlgebra(rbind(cbind(1, -knot, knot, Y_mueta2s - Y_mueta1s),
                                            cbind(0, 1, -1, 0),
                                            cbind(0, 1, 1, 0),
                            ...
                            mxMatrix("Full", 4, length(growth_TIC), free = TRUE, values = starts$beta,
                                     labels = c(paste0("beta0", seq_along(growth_TIC), "s"),
                                                paste0("beta1", seq_along(growth_TIC), "s"),
                                                paste0("beta2", seq_along(growth_TIC), "s"),
                                                paste0("betag", seq_along(growth_TIC), "s")),
      ...
      if (!is.null(growth_TIC)){
        ...
        model_mx <- mxModel(name = "LGCM_TICs, Bilinear Spline Functional Form, Fixed knot", type = "RAM",
                            ...
                            mxMatrix("Full", 3, length(growth_TIC), free = TRUE, values = starts$beta[1:3, ],
                                     labels = c(paste0("beta0", seq_along(growth_TIC), "s"),
                                                paste0("beta1", seq_along(growth_TIC), "s"),
                                                paste0("beta2", seq_along(growth_TIC), "s")), byrow = TRUE, name = "beta_s"),
                            mxMatrix("Full", length(growth_TIC), 1, free = TRUE, values = starts$TIC_starts$means,
                                     labels = c(paste0("mux", seq_along(growth_TIC))), byrow = FALSE, name = "mux"),
